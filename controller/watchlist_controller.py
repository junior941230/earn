...
class WatchlistController:
    ...
    def __init__(self, tab, layout: QGridLayout, db: Database, market_data_service: MarketDataService, websocket_manager: WebSocketManager, all_market_data: dict):
        self.tab = tab
        self.layout = layout
        self.db = db
    ...
    def load_watchlist(self):
        ...
        for i in range(len(watchlists)):
            ...
            self.watchlistGroups[wl_id] = watchlist(
                wl_name, self.edit_watchlistCallback,self.websocket_manager.tradesMessageReceived, wl_id)  # 建立 watchlist UI 元件
            ...
            symbols_in_watchlists.update(self.db.get_watchlist_items(wl_id))  # 收集所有 watchlist 中的 symbol

        self.websocket_manager.maintain_target_cards(symbols_in_watchlists)
